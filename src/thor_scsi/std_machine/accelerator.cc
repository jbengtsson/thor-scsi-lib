 ...
 elements)
{
  ...
  std::transform
    (elements.begin(), elements.end(), std::back_inserter(cells),
     [](std::shared_ptr<thor_scsi::core::ElemTypeKnobbed/*<C>*/> elem)
...
(const Config & conf, bool marker_at_start)
  :tsc::Machine(conf)
{
  if(marker_at_start){
 ...
 int max_elements, size_t n_turns,  bool tracy_compatible_indexing)// const
{
  ...
  for(size_t turn=0; turn<n_turns; ++turn) {
    if(trace)
      (*trace) << "turn " <<  turn << ", processing elements from "
	       << start_elem
	       << " for a maximum of elements " << max_elements << std::endl;

    next_elem = static_cast<int>(start_elem);
    for(int i=0; next_elem >= 0 && next_elem<nelem && i<std::abs(max_elements);
	i++)
      {
	size_t n = next_elem;
	...
	if(observer){
	  observer->view
	    (std::const_pointer_cast<tsc::ElemTypeKnobbed/*<C>*/>(elem), ps,
	     tsc::ObservedState::start, 0);
	}
	...
	if(aperture){
	  ...
	  if(not flag){
	    ...
	    auto rect_apt =
	      std::dynamic_pointer_cast<tse::RectangularAperture>(aperture);
	    if(rect_apt){
	      double x=-1e0, y=-1e0;
	      if(x < 0e0){
	...
	if(trace)
	  (*trace) << "After ["<< n<< "] " << cv->name << " " <<std::endl << ps
		   << std::endl;
      }
  }
  return next_elem;
...
/*
int ts::AcceleratorKnobbable::propagate
(thor_scsi::core::ConfigType& conf, ss_vect_tps  &ps, size_t start,
 int max_elements, size_t n_turns, bool tracy_compatible_indexing)// const
{
  return
    _propagate
...
*/
template<class C>
int ts::AcceleratorKnobbable<C>::propagate
...
template ts::AcceleratorKnobbable
<tsc::StandardDoubleType>::AcceleratorKnobbable
(const Config &conf, bool add_marker_at_start);
template ts::AcceleratorKnobbable
<tsc::TpsaVariantType>::AcceleratorKnobbable
(const Config &conf, bool add_marker_at_start);
template ts::AcceleratorKnobbable
<tsc::StandardDoubleType>::AcceleratorKnobbable
(std::vector<std::shared_ptr<thor_scsi::core::CellVoid>> elements,
...
template
int ts::AcceleratorKnobbable<tsc::StandardDoubleType>::
propagate
