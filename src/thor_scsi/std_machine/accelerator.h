...
namespace thor_scsi {
  ...
  public:
     ...
     *
     * @warning consider if the marker is not better added manually to the
     *          lattice file
     */
    ...
    /*
     */
    AcceleratorKnobbable
    (std::vector<std::shared_ptr<thor_scsi::core::CellVoid>>& elements,
     bool add_marker_at_start=false);

    AcceleratorKnobbable
    (const std::vector<std::shared_ptr<thor_scsi::core::ElemTypeKnobbed>>&
     elements, bool add_marker_at_start=false);
     ...
     * @param max The maximum number of elements through which the state will
     *        be passed
     * @param tracy compatible indexing: start to refer to first element with 1
     *        instead of zero
     * @param add_marker_at_start add a marker at the start of the lattice
     *
     ...
     *
     * @warning tracy compatible indexing will be removed soon as it is not
     *          consistent with global indexing consider if the marker is not
     *          better added manually to the lattice file
     *
     * @todo proper interface design!
     */
    template <typename T>
    int _propagate
    (thor_scsi::core::ConfigType& conf, gtpsa::ss_vect<T>& ps, size_t start,
     int max, size_t n_turns, bool tracy_compatible_indexing = false);

    /*
    int propagate
    (thor_scsi::core::ConfigType&, ss_vect_tps &ps, size_t start=0,
     int max_elements=std::numeric_limits<int>::max(), size_t n_turns=1,
     bool tracy_compatible_indexing = false);
    */
    int propagate
    (thor_scsi::core::ConfigType&, ss_vect_tpsa &ps, size_t start=0,
     int max_elements=max_elements_default, size_t n_turns=1,
     bool tracy_compatible_indexing = false);
    int propagate
    (thor_scsi::core::ConfigType&, ss_vect_dbl &ps, size_t start=0,
     int max_elements=max_elements_default, size_t n_turns=1,
     bool tracy_compatible_indexing = false);
  private:
    /**
     * @brief add a marker at the beginning of the lattice if the lattice does
     *        not start with one
     *
     * @warning rather than using this feature consider adding it manually to
     *          the lattice file.
     *          this here is plagued to add more and more in evolved processing
     *          lines
     */
  ...
  Accelerator;
  typedef class AcceleratorKnobbable<thor_scsi::core::TpsaVariantType>
  AcceleratorTpsa;
}
