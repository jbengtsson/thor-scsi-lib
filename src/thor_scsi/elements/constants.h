...
namespace thor_scsi::elements{
	 ...
	 */
		...
		const double eps_0 = 1.0/(detail::sqr(speed_of_light)*mu_0);   // permeability of free space.
		const double r_e   = q_e/(4.0*M_PI*eps_0*m_e); ///< classical electron radius.

		double r =  4e0 * M_PI * r_e / (3e0 * detail::cube(m_e));
		return r;
	}

        inline double compute_C_q(void) {
	        const double c0 = speed_of_light;
