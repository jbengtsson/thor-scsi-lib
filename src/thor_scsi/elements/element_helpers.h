...
extern double q_fluct; /* < Track down if it is a constant or a global
			  variable reflecting current status                  */
 ...
 */
	template<typename T>
	inline T get_p_s
	(const thor_scsi::core::ConfigType &conf, const gtpsa::ss_vect<T> &ps)
	{
		...
		if (!conf.H_exact) {
			...
			p_s2 =
				sqr(1e0+ps[delta_]) - sqr(ps[px_])
				- sqr(ps[py_]);
			if (p_s2 >= 0e0){
				p_s = sqrt(p_s2);
			}  else {
				throw PhysicsViolation
...
#warning "optimise dI eta"
	...
	static inline double get_dI_eta(const gtpsa::ss_vect<double> &A){
		std::cout << "get_dI_eta: operation not defined for double"
			  << std::endl;
		throw std::domain_error
			("get_dI_eta: operation not defined for tps");
		return 0e0;
	}
 ...
 */
	...
	public:
		 ...
		 */
		/*
		static inline void get_ps
		(const ss_vect<double> &x, thor_scsi::core::CellType *Cell)
		...
		*/
		 ...
		 */
		static inline double get_dI_eta(const gtpsa::ss_vect<tps> &A){
			std::cout << "get_dI_eta: operation not defined for"
				" double" << std::endl;
			throw std::domain_error
				("get_dI_eta: operation not defined for tps");
			return 0e0;
		}
		static inline double get_dI_eta
		(const gtpsa::ss_vect<gtpsa::tpsa> &A){
			std::cout << "get_dI_eta: operation not defined for"
				" gtpsa::tpsa" << std::endl;
			throw std::domain_error
				("get_dI_eta: operation not defined for"
				 " gtpsa::tpsa");
			return 0e0;
		}


		static inline void diff_mat
		(const double B2, const double u, const double ps0,
		 const gtpsa::ss_vect<double> &xp) { }
...
// partial specialization
	...
	public:
		/*
		static inline void get_ps
		(const ss_vect<tps> &x, thor_scsi::core::CellType *Cell)
			{ Cell->BeamPos = pstostl(x.cst());
				Cell->A = maptostlmat(x); }
		*/
		...
		/*
		 * dN<u^2>/E^2 =
		 *	3*C_U*C_gamma*h_bar*c*E_0^5*(1+delta)^4
		 *	*(B_perp/(Brho))^3/(4*pi*m_e^3 [eV/c^2])
		 */
		/**
		 *
		 @f[
			 ...
			 \frac{\left(\frac{B_{perp}}{B\rho}\right)^3}
			 {(4*pi*m_e^3 [eV/c^2])}
		 * @f]
		 ...
		 * see [Sands]_
		 *
		 * .. [Sands] M. Sands "The Physics of Electron Storage Rings"
		 *    SLAC-121, Eq. (5.20), p. 118
		 *
		 * \endverbatim
		 */

		static inline void diff_mat
		(const tps &B2_perp, const tps &ds, const tps &p_s0,
		 gtpsa::ss_vect<tps> &x)
			{ }
	};
	/**
	 ...
	 * See drift.h for DriftType, which is an implementation as separate
	 * lattice lement
	 */
	template<typename T, typename T2>
	void drift_propagate
	(const thor_scsi::core::ConfigType &conf, const T2& L,
	 gtpsa::ss_vect<T> &ps);

	/**
	 * @brief implementation of the thin kick (thin lens approximation)
	 *
	 *   @param h_bend: 1/rho_bend (radius of the the bend, magnetic
	 *                              rigidty of the dipoles)
	 *   @param h_ref:  1/rho which is the curvature of the design
	 *                  / reference orbit
	 *
	 * \verbatim embed:rst:leading-asterisk
	 *
	 ...
	 *     * Consider to combine interpolation with length L and thus not
	 *       using field interpolation but field integral interpolation
	 *     * consider renaming function to thin lens
	 *
	 *
	 * .. Warning::
	 *
	 *    * cartesion bend with evaluation of gradients not supported
	 *      curently (code not active
	 *
	 * The vector potential for the combined-function sector bend is from:
	 * [Iselin:85]_
	 *
	 * .. [Iselin:85]  C. Iselin "Lie Transformations and Transport
	 *    Equations for Combined-Function Dipoles" Part. Accel. 17,
	 *    143-155 (1985).
	 * \endverbatim
	 */
	template<typename T>
