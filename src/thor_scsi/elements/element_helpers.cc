...
namespace thor_scsi::elements{
  ...
  double get_curly_H(const gtpsa::ss_vect<double> &x){
    // THOR_SCSI_LOG(THOR_SCSI_WARN) << "get_curly_H:
  ...
  (const tsc::ConfigType &conf, const T2& L, gtpsa::ss_vect<T> &ps)
  {
    T u(ps[0]);
   ...
   *
   * Args:
   *   order: optimisation for number of coefficients that require to be
   *           evaulatued
   *          should go to field interpolation
   *
   *   h_bend: 1/rho_bend (radius of the the bend, magnetic rigidty of the
   *                       dipoles)
   *   h_ref: $1 / \rho$ for the reference curve
   *   / for the comoving frame (curvature of the design or reference orbit)
   *
		 ...
		 const gtpsa::ss_vect<T> &ps0,  gtpsa::ss_vect<T> &ps)
  {
    ...
    if(debug){
      std::cerr << "Length " << L << " Order " << Order << " h_bend "
		<< h_bend << " h_ref " << h_ref << std::endl << "ps in " << ps;
    }
	...
	// Need to understand
	if(debug){
	  std::cerr << "interpolated field " << BxoBrho << ", "  << ByoBrho
		    << std::endl;
	}
...
template void tse::drift_propagate
(const tsc::ConfigType &conf, const double&, gtpsa::ss_vect<double>      &);
template void tse::drift_propagate
(const tsc::ConfigType &conf, const double&, gtpsa::ss_vect<tps>         &);
template void tse::drift_propagate
(const tsc::ConfigType &conf, const double&, gtpsa::ss_vect<gtpsa::tpsa> &);


template void tse::thin_kick
(const tsc::ConfigType &conf, const double BxoBrho, const double ByoBrho,
 const double L, const double h_bend, const double h_ref,
 const gtpsa::ss_vect<double> &ps0, gtpsa::ss_vect<double> &ps);
template void tse::thin_kick
(const tsc::ConfigType &conf, const tps BxoBrho, const tps ByoBrho,
 const double L, const double h_bend, const double h_ref,
 const gtpsa::ss_vect<tps> &ps0, gtpsa::ss_vect<tps> &ps);
template void tse::thin_kick
(const tsc::ConfigType &conf, const gtpsa::tpsa BxoBrho,
