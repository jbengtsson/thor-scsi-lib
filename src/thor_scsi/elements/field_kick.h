...
namespace thor_scsi::elements {
	...
	public:
		...
		inline std::unique_ptr<std::vector<double>> getDriftLength(void)
			const {
		...
		inline std::unique_ptr<std::vector<double>> getKickLength(void)
			const {
			auto res = std::make_unique<std::vector<double>>(2);
			res->at(0) = this->dL1;
			res->at(1) = this->dL2;
	...
	public:
		...
		/**
		 * @Todo: clarify if angle should be treated in radian or
		 *        degree
		 *
		...
		(thor_scsi::core::ConfigType &conf, gtpsa::ss_vect<double>      &ps) override final { _localPropagate(conf, ps);}
		virtual void localPropagate
		(thor_scsi::core::ConfigType &conf,
		 gtpsa::ss_vect<gtpsa::tpsa> &ps) override final {
			_localPropagate(conf, ps);
		}
		/*
		  virtual void localPropagate
		  (thor_scsi::core::ConfigType &conf,
		  gtpsa::ss_vect<tps> &ps) override final {
		  // _localPropagate(conf, ps);
		  throw std::runtime_error
		  ("field kick local propagate not implemented for"
		   " ss_vect<tps> ");
		  }
		*/
	...
	private:
		...
		template<typename T>
		void _localPropagateThin
		(const thor_scsi::core::ConfigType &conf,
		 gtpsa::ss_vect<T> &ps);

		template<typename T>
		void _localPropagateBody
		(thor_scsi::core::ConfigType &conf, gtpsa::ss_vect<T> &ps);
		...
		void inline validateIntegrationMethod(const int n) const {
			...
			default:
				...
				strm << "Only implemented integration method 4"
					" but found " << n;
	...
	public:
		 ...
		 */
	public:
		typedef thor_scsi::elements::RadiationDelegateKickKnobbed
		 ...
		 gtpsa::ss_vect<T> &ps){
			if(this->computeSynchrotronIntegrals(conf)){
				...
				if(ob){
					ob->view(*this, ps,
						 thor_scsi::core
						 ::ObservedState::start, 0);
				}
			}
		}

		// first step of synchrotron integrals increment
		// / local contribution
		// rename it to
		template<typename T>
