...
namespace thor_scsi::elements {
	template<class C, typename = typename C::complex_type,
		 typename = typename C::double_type> // CK for complex knob
	...
	public:
		 ...
		 * \endverbatim
		 */
		inline MpoleTypeWithKnob(const Config &config) :
			FieldKickKnobbed<C>(config){
			auto tmp = std::make_shared<multipoles_knobbed>(0e0);
			this->intp =
				std::move
				(std::dynamic_pointer_cast
				 <thor_scsi::core::Field2DInterpolationKnobbed
				 <C>>(tmp));
			/*
			std::cerr << "Mpole Type: " << std::setw(10) << name
				  << " interpolation object " << this->intp
				  << std::endl;
			auto parent = dynamic_cast<FieldKick*>(this);
			std::cerr << "FieldKick: interpolation object "
				  << parent->getFieldInterpolator()
				  << std::endl;
			*/
		}
		...
		inline auto getFieldInterpolator(void) const {
			...
			strm << __FILE__
			     << ":mpole.getFieldInterpolator: this->intp "
			     << static_cast<void *>(this->intp.get());
			auto p = std::dynamic_pointer_cast<multipoles_knobbed>
				(this->intp);
			...
			auto cp =
				std::const_pointer_cast<multipoles_knobbed>(p);
		...
		inline void setFieldInterpolator
		(std::shared_ptr<thor_scsi::core::
		 TwoDimensionalMultipolesKnobbed<C>> intp) {
			auto p =
				std::dynamic_pointer_cast
				<thor_scsi::core::Field2DInterpolationKnobbed
				 <C>>(intp);
			if(!p){
				throw std::runtime_error
					("Could not cast multipole to"
					 " Field2DInterpolation");
			}
			this->intp = p;
		}
