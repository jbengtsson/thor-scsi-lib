...
namespace thor_scsi::elements {
	...
	public:
		...
		(std::shared_ptr<thor_scsi::core::Field2DInterpolationKnobbed
		 <C>> a_intp){
			this->intp = a_intp;
		}
			...
			*/
			if(!this->intp){
				throw std::logic_error
					("interpolator pointer null");
			}
			/*
			std::cerr << "use count " << this->intp.use_count()
				  << std::endl;
			std::cerr.flush();
			*/
			auto tmp = std::const_pointer_cast
	...
	protected:
		...
		std::shared_ptr
		<thor_scsi::core::Field2DInterpolationKnobbed<C>> intp;
		int integration_steps = 1;
	};

	typedef FieldKickAPIKnobbed<thor_scsi::core::StandardDoubleType>
	FieldKickAPI;
} // Name space
