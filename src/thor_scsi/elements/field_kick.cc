...
namespace thor_scsi::elements {
  template<typename T>
  void edge_focus
  (const tsc::ConfigType &conf, const double irho,
   const double phi /* in degrees */, const double gap, gtpsa::ss_vect<T> &ps);
...
void tse::FieldKickForthOrder<C>::computeIntegrationSteps(void)
{
  ...
  const double
    Pirho = this->parent->getCurvature(), length = this->parent->getLength();
  double dL;
  auto n_steps = this->getNumberOfIntegrationSteps();
  // std::cerr << __FILE__ << " Field kick forth order: using " << n_steps
  // 	    << " n steps" << std::endl;
...
template<class C>
void tse::FieldKickForthOrder<C>::splitIntegrationStep
(const double dL, double *dL1, double *dL2, double *dkL1, double *dkL2)
  const
{
...
(tsc::ConfigType &conf, gtpsa::ss_vect<T> &ps)
{
  ...
  // std::cerr << __FILE__
  // 	    << " field kick local propagate:: number of integration steps "
  // 	    << PN << std::endl;
...
#error "Only compiling with synchrotron integrals"
  ...
  for (int seg = 1; seg <= PN; seg++) {
    // std::cerr << __FILE__ << ": 4th order integration step " << seg << "/"
    // 	      << PN << std::endl;
...
(const thor_scsi::core::ConfigType &conf,
 const thor_scsi::core::Field2DInterpolationKnobbed<C>& intp, const double L,
 const double h_bend, const double h_ref, gtpsa::ss_vect<T> &ps)
{
  ...
  */
  ...
  // THOR_SCSI_LOG(DEBUG) << "\n  thinKickAndRadiate ->: B = (x=" << BxoBrho
  // 		       << ", y=" << ByoBrho << ") \n";
...
template void tse::FieldKickForthOrder<StandardDoubleType>::
computeIntegrationSteps(void);
template void tse::FieldKickForthOrder<TpsaVariantType>::
computeIntegrationSteps(void);
...
template void tse::FieldKickKnobbed<StandardDoubleType>::show
(std::ostream &strm, const int level) const;
template void tse::FieldKickKnobbed<TpsaVariantType>::show
(std::ostream &strm, const int level) const;

template tse::FieldKickKnobbed<StandardDoubleType>::FieldKickKnobbed
(const Config &config);
template tse::FieldKickKnobbed<StandardDoubleType>::FieldKickKnobbed
(thor_scsi::elements::FieldKickKnobbed<StandardDoubleType> &&);

template tse::FieldKickKnobbed<TpsaVariantType>::FieldKickKnobbed
(const Config &config);
template tse::FieldKickKnobbed<TpsaVariantType>::FieldKickKnobbed
