...
void check_close
(const double expected, const double observed, const double small,
 const double pct_tol)
{
...
BOOST_AUTO_TEST_CASE(test62_rotate_dipole)
{
...

  {
    ...
    BOOST_CHECK_CLOSE
      ((std::arg(field0) + phase) * rad2deg, std::arg(check0) * rad2deg, 1e-10);
    BOOST_CHECK_CLOSE
      ((std::arg(field1) + phase) * rad2deg, std::arg(check1) * rad2deg, 1e-10);
  }
  {
    ...
    check_close
      ((std::arg(rpos0) + angle) * rad2deg, std::arg(pos0) * rad2deg, 1e-13,
       1e-13);
    BOOST_CHECK_CLOSE
      ((std::arg(rpos1) + angle) * rad2deg, std::arg(pos1) * rad2deg, 1e-13);
