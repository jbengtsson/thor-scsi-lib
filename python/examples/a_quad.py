...
nv = 6
desc = gtpsa.desc(nv, mo, len(index_dict) - nv)
...
dx = gtpsa.tpsa(desc, mo, mapping=index_mapping)
dy = gtpsa.tpsa(desc, mo, mapping=index_mapping)
if False:
    b2.set_variable(k + 0j, "K")
    dx.set_variable(0, "dx")
    dy.set_variable(0, "dy")
else:
    b2.set_knob(k + 0j, "K")
    dx.set_knob(0, "dx")
    dy.set_knob(0, "dy")
...
quadK.set_dx(gtpsa.TpsaOrDouble(dx))
quadK.set_dy(gtpsa.TpsaOrDouble(dy))


# Index mapping should not be necessary
ps = gtpsa.ss_vect_tpsa(desc, mo, nv, index_mapping)
ps.set_identity()
