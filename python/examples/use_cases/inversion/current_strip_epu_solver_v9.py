#!/usr/bin/env python3
"""
current_strip_epu_solver_v9.py
...
RELEASE_ID = "EPU57-SOLVER-20-ORTHOGONAL-STRIP-ARRAYS"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "default encloses the complete input x grid with one strip-width "
            "margin on each side"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        metavar="N",
        help=(
            "number of strips on each top/bottom horizontal sheet; centres "
            "span ±x-range-mm (default: 21)"
        ),
    )
    parser.add_argument(
    ...
    if arguments.strip_layout in {"horizontal", "both"} and (
        arguments.gap / 2.0 <= maximum_map_y_m
    ):
        raise ValueError(
            "The top/bottom clear gap must enclose the complete kick-map y "
            "grid; increase --gap"
        )

    if arguments.side_gap is None:
        effective_side_gap_m = 2.0 * (
            maximum_map_x_m + arguments.width
        )
        side_gap_source = "auto_full_x_grid_plus_one_width_margin"
    else:
        effective_side_gap_m = arguments.side_gap
        side_gap_source = "user"
    if arguments.strip_layout in {"vertical", "both"} and (
        effective_side_gap_m / 2.0 <= maximum_map_x_m
    ):
        raise ValueError(
            "The left/right clear gap must enclose the complete kick-map x "
            "grid; increase --side-gap"
        )

    strips, strip_geometry = make_strip_arrays(
        layout=arguments.strip_layout,
        horizontal_half_span_m=arguments.x_range_mm * 1.0e-3,
