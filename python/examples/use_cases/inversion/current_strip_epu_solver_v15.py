...
RELEASE_ID = "CURRENT-STRIP-SOLVER-26-SI-RANGE-OPTIONS"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    geometry.add_argument(
        "--vertical-strip-range",
        type=float,
        default=None,
        metavar="M",
        help=(
            "outermost vertical strip-centre positions ±M in metres; "
    ...
    aperture.add_argument(
        "--x-range",
        type=float,
        default=0.020,
        metavar="M",
        help="model half-aperture |x| <= M in metres",
    )
    aperture.add_argument(
        "--y-range",
        type=float,
        default=None,
        metavar="M",
        help="model half-aperture |y| <= M in metres; default uses the full input grid",
    )
...
def validate_arguments(args: argparse.Namespace) -> argparse.Namespace:
    for name in ("kick_scale", "x_range", "gap", "width", "thickness", "limit"):
        _require_finite(name.replace("_", "-"), getattr(args, name), positive=True)
    _require_finite("y-range", args.y_range, positive=True, allow_none=True)
    ...
    _require_finite(
        "horizontal-strip-range", args.horizontal_strip_range, positive=True
    )
    ...
    _require_finite(
        "vertical-strip-range", args.effective_vertical_strip_range,
        positive=True,
    )
...
class CurrentStripWorkflow:
    ...
    def _prepare(self) -> None:
        ...
        self.model_y_half_span_m = (
            self.maximum_map_y_m if a.y_range is None else a.y_range
        )
        ...
        if a.strip_layout in {"vertical", "both"} and self.effective_side_gap_m / 2.0 <= a.x_range:
            raise ValueError(
    ...
    def _write_outputs(self, metrics_text: str) -> str:
        ...
        with tempfile.TemporaryDirectory(
            ...
            plot_args = dict(
                x_range_m=a.x_range, y_range_m=a.y_range,
