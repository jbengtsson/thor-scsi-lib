#!/usr/bin/env python3
"""
current_strip_epu_solver_v7.py
...
21. --target-mode nonlinear (the default) always projects the constant
    steering term out of each target cut and every strip-response column.
    --horizontal-gradient and --vertical-gradient then select independently
    whether each linear gradient is retained or projected out.  The exported
    map is still the complete physical map; its remaining affine coefficients
    are recorded for subsequent lattice correction.  --target-mode full
    restores the complete full-kick objective.
...
23. --horizontal-gradient and --vertical-gradient independently select
    whether the corresponding linear gradient is included in the nonlinear
    fit.  In nonlinear mode the constant steering term is always removed;
    gradient=exclude also removes the linear term, while gradient=include
    retains the linear term together with the nonlinear residual.  Defaults
    are exclude/exclude, preserving the previous nonlinear-target behavior.
...
RELEASE_ID = "EPU57-SOLVER-18-INDEPENDENT-GRADIENT-CONTROL"
...
class ConstantDecomposition:
    """Mean (steering) and mean-removed parts along the first axis."""
...
def constant_decomposition(values: np.ndarray) -> ConstantDecomposition:
    """Split values into a constant steering term and a mean-removed part.

    The first axis is treated as the sampled coordinate.  For a matrix, the
    mean is removed independently from every response column.  This basis is
    used when a component's gradient is included in the fit: the constant
    steering term remains excluded, while the linear gradient and all higher
    orders remain available to the optimizer.
    """
    array = np.asarray(values, dtype=float)
    if array.ndim < 1 or array.shape[0] < 1:
        raise ValueError("constant projection requires a non-empty first axis")
    if not np.all(np.isfinite(array)):
        raise ValueError("constant projection inputs must be finite")

    offset = np.mean(array, axis=0)
    fitted = np.broadcast_to(offset, array.shape)
    residual = array - fitted

    flattened = array.reshape(array.shape[0], -1)
    residual_flat = residual.reshape(array.shape[0], -1)
    column_scale = np.max(np.abs(flattened), axis=0)
    tolerance = (
        256.0
        * np.finfo(float).eps
    ...
    return ConstantDecomposition(
        fitted=np.asarray(fitted, dtype=float),
        residual=residual_flat.reshape(array.shape),
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="nonlinear",
        help=(
            "fit target: 'nonlinear' always removes the constant steering "
            "term and uses --horizontal-gradient/--vertical-gradient to "
            "include or exclude each linear gradient (default); 'full' fits "
            "the complete cuts and therefore includes both gradients and "
            "constant terms"
        ),
    )
    parser.add_argument(
        ...
        default="exclude",
        help=(
            "in nonlinear mode, include or exclude the linear x-gradient "
            "of theta_x(x,y=0); the constant steering term is always "
            "excluded (default: exclude)"
        ),
    )
    parser.add_argument(
        "--vertical-gradient",
        ...
        default="exclude",
        help=(
            "in nonlinear mode, include or exclude the linear y-gradient "
            "of theta_y(x=0,y); the constant steering term is always "
            "excluded (default: exclude)"
        ),
    )
    ...
    target_iy_constant = constant_decomposition(target_iy_tm)
    target_ix_constant = constant_decomposition(target_ix_tm)
    response_iy_constant = constant_decomposition(response_iy_midplane)
    response_ix_constant = constant_decomposition(response_ix_centerline)

    if arguments.target_mode == "nonlinear":
        if arguments.horizontal_gradient == "exclude":
            ...
            fit_target_iy_tm = target_iy_constant.residual
            fit_response_iy = response_iy_constant.residual
            horizontal_fit_basis = "constant_removed_gradient_included"

        if arguments.vertical_gradient == "exclude":
            ...
            fit_target_ix_tm = target_ix_constant.residual
            fit_response_ix = response_ix_constant.residual
            vertical_fit_basis = "constant_removed_gradient_included"
    else:
    ...
    raw_theta_x_constant = constant_decomposition(raw_theta_x_y0.values)
    corrected_theta_x_constant = constant_decomposition(
        corrected_theta_x_y0.values
    )
    raw_theta_y_constant = constant_decomposition(raw_theta_y_x0.values)
    corrected_theta_y_constant = constant_decomposition(
        corrected_theta_y_x0.values
    )
    ...
    raw_theta_x_constant_removed_rms = root_mean_square(
        raw_theta_x_constant.residual
    )
    corrected_theta_x_constant_removed_rms = root_mean_square(
        corrected_theta_x_constant.residual
    )
    (
        horizontal_constant_removed_rms_reduction_factor,
        horizontal_constant_removed_ratio_status,
    ) = safe_reduction_factor(
        raw_theta_x_constant_removed_rms,
        corrected_theta_x_constant_removed_rms,
    )
    ...
    raw_theta_y_constant_removed_peak = float(
        np.max(np.abs(raw_theta_y_constant.residual))
    )
    corrected_theta_y_constant_removed_peak = float(
        np.max(np.abs(corrected_theta_y_constant.residual))
    )
    (
        vertical_constant_removed_peak_reduction_factor,
        vertical_constant_removed_ratio_status,
    ) = safe_reduction_factor(
        raw_theta_y_constant_removed_peak,
        corrected_theta_y_constant_removed_peak,
    )
    ...
    if arguments.target_mode == "nonlinear":
        if arguments.horizontal_gradient == "exclude":
            ...
            horizontal_acceptance_rms_basis = (
                "constant_and_linear_removed"
            )
        else:
            horizontal_acceptance_reduction_factor = (
                horizontal_constant_removed_rms_reduction_factor
            ...
            horizontal_acceptance_rms_basis = (
                "constant_removed_gradient_included"
            )
    else:
        horizontal_acceptance_reduction_factor = (
    ...
    metrics = (
        ...
        f"horizontal_gradient={arguments.horizontal_gradient}\n"
        f"vertical_gradient={arguments.vertical_gradient}\n"
        ...
        f"raw_theta_x_y0_constant_removed_rms_urad="
        f"{raw_theta_x_constant_removed_rms:.12g}\n"
        f"corrected_theta_x_y0_constant_removed_rms_urad="
        f"{corrected_theta_x_constant_removed_rms:.12g}\n"
        f"horizontal_constant_removed_rms_reduction_factor="
        f"{horizontal_constant_removed_rms_reduction_factor:.12g}\n"
        f"horizontal_constant_removed_rms_reduction_status="
        f"{horizontal_constant_removed_ratio_status}\n"
        ...
        f"raw_theta_y_x0_constant_removed_peak_urad="
        f"{raw_theta_y_constant_removed_peak:.12g}\n"
        f"corrected_theta_y_x0_constant_removed_peak_urad="
        f"{corrected_theta_y_constant_removed_peak:.12g}\n"
        f"vertical_constant_removed_peak_reduction_factor="
        f"{vertical_constant_removed_peak_reduction_factor:.12g}\n"
        f"vertical_constant_removed_peak_reduction_status="
        f"{vertical_constant_removed_ratio_status}\n"
    ...
    with tempfile.TemporaryDirectory(
        ...
        if arguments.target_mode == "nonlinear":
            if arguments.horizontal_gradient == "exclude":
                ...
                theta_x_plot_raw = raw_theta_x_constant.residual
                theta_x_plot_corrected = corrected_theta_x_constant.residual
                theta_x_plot_title = (
                    "Horizontal kick on y = 0 "
                    "(constant removed; gradient included)"
                )
        else:
        ...
        if arguments.target_mode == "nonlinear":
            if arguments.vertical_gradient == "exclude":
                ...
                theta_y_plot_raw = raw_theta_y_constant.residual
                theta_y_plot_corrected = corrected_theta_y_constant.residual
                theta_y_plot_title = (
                    "Vertical kick on x = 0 "
                    "(constant removed; gradient included)"
                )
        else:
