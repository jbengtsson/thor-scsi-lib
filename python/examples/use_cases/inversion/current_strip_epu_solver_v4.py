#!/usr/bin/env python3
"""
current_strip_epu_57_solver_15.py
...
20. --horizontal-weight and --vertical-weight set independent non-negative
    ...
    by its number of sampled cut points.  Defaults (1, 0) preserve the R14
    horizontal-only fit.
...
RELEASE_ID = "EPU57-SOLVER-15-WEIGHTED-HV-FIT"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "residual; weights are normalized by their maximum; 0 "
            "preserves the R14 horizontal-only fit (default: 0.0)"
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "weighted RMS target (default: 0.05)"
        ),
    )
    parser.add_argument(
        ...
        metavar="FACTOR",
        help=(
            "minimum accepted raw/corrected horizontal y=0 RMS ratio "
            "(default: 1.0, so correction may not worsen the RMS)"
        ),
    )
    parser.add_argument(
    ...
    if not np.isfinite(arguments.max_fit_relative_rms) or (
        arguments.max_fit_relative_rms < 0.0
    ):
    ...
    if normalized_horizontal_weight > 0.0:
        horizontal_scale = np.sqrt(
            normalized_horizontal_weight / target_iy_tm.size
        )
        weighted_response_blocks.append(horizontal_scale * response_iy_midplane)
        weighted_target_blocks.append(horizontal_scale * target_iy_tm)
    if normalized_vertical_weight > 0.0:
        vertical_scale = np.sqrt(
            normalized_vertical_weight / target_ix_tm.size
        )
        weighted_response_blocks.append(vertical_scale * response_ix_centerline)
        weighted_target_blocks.append(vertical_scale * target_ix_tm)
    ...
    fit_prediction_iy_tm = response_iy_midplane @ currents_a
    fit_residual_iy_tm = fit_prediction_iy_tm - target_iy_tm
    fit_prediction_ix_tm = response_ix_centerline @ currents_a
    fit_residual_ix_tm = fit_prediction_ix_tm - target_ix_tm

    horizontal_fit_target_rms_tm = root_mean_square(target_iy_tm)
    horizontal_fit_residual_rms_tm = root_mean_square(fit_residual_iy_tm)
    horizontal_fit_relative_rms = relative_rms_residual(
        fit_residual_iy_tm,
        target_iy_tm,
    )
    vertical_fit_target_rms_tm = root_mean_square(target_ix_tm)
    ...
    if fit_relative_rms > arguments.max_fit_relative_rms:
        acceptance_failures.append(
            "fit relative RMS "
            f"{fit_relative_rms:.6g} exceeds "
            f"{arguments.max_fit_relative_rms:.6g}"
        )
    if arguments.horizontal_weight > 0.0 and (
        ...
        acceptance_failures.append(
            ...
            f"{horizontal_rms_reduction_factor:.6g} is below "
    ...
    metrics = (
        ...
        f"{arguments.max_fit_relative_rms:.12g}\n"
    ...
    with tempfile.TemporaryDirectory(
        ...
        y_cut_m, raw_theta_y_cut_urad = crop_vertical_range(
            kick_map.y_m,
            raw_theta_y_x0.values[:, np.newaxis],
            arguments.y_range_mm,
        )
        _, corrected_theta_y_cut_urad = crop_vertical_range(
            kick_map.y_m,
            corrected_theta_y_x0.values[:, np.newaxis],
            arguments.y_range_mm,
        )
        plot_cut(
            y_cut_m * 1.0e3,
