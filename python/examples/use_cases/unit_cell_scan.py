...
def plt_eps_x_alpha_c(file_name, phi, eps_x, alpha_c, plot):
    ...
    gr_1_r = gr_1.twinx()
    gr_1_r.set_ylabel(r"$\alpha_c$ [1e-4]")
    gr_1_r.plot(
...
def unit_cell_rev_bend(lin_opt, rad_prop, n_step, phi_min):
    # Dipoles:
    ...
    eps_x_buf = []
    alpha_c_buf = []
    print("\n   phi   phi_tot   eps_x     alpha_c      eta_x"
          "\n  [deg]   [deg]  [nm.rad]                  [m]")
    for k in range(n_step):
        ...
        alpha_c = lin_opt.compute_alpha_c()
        if alpha_c > 0e0:
        ...
        print("{:7.3f}  {:5.3f}    {:5.1f}   {:10.3e}  {:10.3e}".
              format(
                  phi_rb, compute_phi(lin_opt._lattice), 1e12*rad_prop._eps[X_],
                  alpha_c, eta_x))
...
home_dir = os.path.join(
    ...
    "ipac_2024")
file_name = os.path.join(home_dir, "2024Mar_tracy.lat")

prt_default_mapping()

lin_opt = ps.lin_opt_class(nv, no, nv_prm, no_prm, file_name, E_0, cod_eps)
rad_prop = rp.rad_prop_class()
...
if not False:
    phi, eps_x, alpha_c = unit_cell_rev_bend(lin_opt, rad_prop, 10, -1.1)
