...
design_val = {
    "eps_x_des" : 50e-12,
    "nu_uc_des" : np.array([2.0/5.0, 0.5/5.0]),
    "eta_x_des" : 1e-4
}
...
def get_prms(lat_prop, bend_list, eps):
    prm = [
        ("QF1",        "b_2",      prm_range["b_2"]),
...
uc_centre = lat_prop._lattice.find("SF_F", 0).index
...
prm_list, dprm_list = get_prms(lat_prop, bend_list, 1e-4)
