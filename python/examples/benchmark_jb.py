"""Use Case prototype code for: Compute dynamic aperture.
"""
...
def track(acc, conf, params, x, px, y, py, delta, f_rf, prt):
    """6D phase-space tracking for given initial conditions & parameters params:
    n_track_DA no of turns
    """
    ps = ss_vect_double

    [ps[x_], ps[px_], ps[y_], ps[py_], ps[delta_], ps[ct_]] = \
        [x, px, y, py, delta, 0e0]

    if prt:
        f = open("track.out", mode='w')
        print("# Tracking with Thor\n", file=f)
        print("#\n", file=f)
        print("#  n       x           p_x          y            p_y  "
              "       delta         cdt\n", file=f)
        if f_rf == 0e0:
            print("#         [mm]        [mrad]       [mm]         [mrad]"
	          "                    [mm]\n", file=f)
            print(f"{0:4d}{1e3*ps[x_]:24.16e}{1e3*ps[px_]:24.16e}{1e3*ps[y_]:24.16e}{1e3*ps[py_]:24.16e}{1e2*ps[delta_]:24.16e}{1e3*ps[ct_]:24.16e}\n", file=f)
        else:
            print("#         [mm]        [mrad]       [mm]         [mrad]"
	    << "                    [deg]\n", file=f)
            print(f"{0:4d}{1e3*ps[x_]:24.16e}{1e3*ps[px_]:24.16e}{1e3*ps[y_]:24.16e}{1e3*ps[py_]:24.16e}{1e2*ps[delta_]:24.16e}{ 2.0*f_rf*180.0*ps[ct_]/c0:24.16e}\n", file=f)
            print("#", file=f)

    for i in range(1, params.n_track_DA+1):
#        acc.propagate(conf, ps, s_loc)
        acc.propagate(conf, ps)

        s_loc = globval.Cell_nLoc
        if s_loc == globval.Cell_nLoc:
            if prt:
                if f_rf == 0e0:
                    print(f"{i:4d}{1e3*ps[x_]:24.16e}{1e3*ps[px_]:24.16e}{1e3*ps[y_]:24.16e}{1e3*ps[py_]:24.16e}{1e2*ps[delta_]:24.16e}{1e3*ps[ct_]:24.16e}\n", file=f)
                else:
                    print(f"{i:4d}{1e3*ps[x_]:24.16e}{1e3*ps[px_]:24.16e}{1e3*ps[y_]:24.16e}{1e3*ps[py_]:24.16e}{1e2*ps[delta_]:24.16e}{ 2.0*f_rf*180.0*ps[ct_]/c0:24.16e}\n", file=f)
        else:
            return false
    if prt:
        f.close()
...
def DA_data_type::get_r_stable(const param_data_type &params,
			       ...
			       const double eps, double x_min[], double x_max[])
{
  ...
  for (i = 0 i < params.n_aper_DA i++) {
    ...
    x2[X_] = r1*cos(phi)
          x2[Y_] = r1*sin(phi)
    for (j = 0 j <= 1 j++) {
      x_min[j] = min(x2[j], x_min[j])
            x_max[j] = max(x2[j], x_max[j])
    }
    if (i == 0) {
      x0[X_] = x2[X_]
            x0[Y_] = x2[Y_]
...
void DA_data_type::get_DA_bare(param_data_type &params)
{
  ...
  for (j = 0; j <= params.n_delta_DA; j++) {
    ...
    sprintf(str, "DA_bare_%4.2f.out", 1e2*d);
          fp = file_write(str);
    ...
    x_hat[X_] = (x_max[X_]-x_min[X_])/2.0;
          x_hat[Y_] = x_max[Y_];
			       ...
			       orb_corr_type orb_corr[])
{
  ...
  for (j = 1; j <= params.n_stat; j++) {
	...
	x_hat_m[k][X_] += x_hat;
              x_hat_s[k][X_] += sqr(x_hat);
	x_hat = x_max[Y_];
	x_hat_m[k][Y_] += x_hat;
              x_hat_s[k][Y_] += sqr(x_hat);
      }
