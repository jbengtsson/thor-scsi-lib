...
static auto declare_ss_vect(py::module &m, const std::string pyclass_name) {
	...
	a_class
		...
		/*
		.def("__eq__",       [](const ss_vect<T> &self,
					(const ss_vect<T> &a)){
		...
		*/
		...
		.def("__len__",      [](const ss_vect<T> &a) -> int
			{return ps_dim;})
		...
		.def("__setitem__",
		     [](ss_vect<T> &a, const int k, const double &x){
			     ss_vect_check_index(k);
			     a.at(k) = x;
		})
...
static void ss_vect_tps_check_coor(const int coor)
{
	if(coor < 1){
		...
		strm << "Coordinate (first index) must be at least 1 but was "
		     << coor;
		throw py::index_error(strm.str());
	}
	if(coor > ps_dim) {
		std::ostringstream strm;
		strm << "Coordinate (first index) must be at less than "
		     << ps_dim << " but was " << coor;
		throw py::index_error(strm.str());
	}
}
...
void py_thor_scsi_init_tps(py::module &m)
{
	...
	// m.def("partialInverse", &PInv,
	//       "partial inverse depending on the numbers of freedoms");
	m.def("partialInverse",  py::overload_cast<const ss_vect<tps>&,
	      const tpsa_index&>(&PInv),
	      "partial inverse depending on the numbers of freedoms");
	m.def("select_subpart", &select_subpart,
	      "support experiment to understand the code");
	...
	ss_vect_tps
		.def("__getitem__",
		     [](ss_vect<tps> &ps, py::sequence &seq) -> double {
			     py::int_ coord= seq[0], term =  seq[1];
			     const int c = coord, t = term;
			     // std::cerr << "accessing element coord " << c <<
			     // 	     " term " << t << std::endl;
		...
		.def("__setitem__",
		     [](ss_vect<tps> &ps, py::sequence &seq, const double val) {
			     py::int_ coord= seq[0], term =  seq[1];
			     const int c = coord, t = term;
			     // std::cerr << "accessing element coord " << c <<
			     // 	     " term " << t << std::endl;
