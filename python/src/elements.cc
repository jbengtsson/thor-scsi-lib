...
class PyFieldKick: public tse::FieldKick {
	...
	void show(std::ostream& strm, int level) const override {
		const std::string txt = (level <= 1) ?
			this->pstr() : this->repr();
		strm << txt;
	}

	std::string repr(void) const {
		PYBIND11_OVERRIDE_NAME(std::string, tsc::FieldKick, "__repr__",
				       repr);
	}

	std::string pstr(void) const {
		PYBIND11_OVERRIDE_NAME(std::string, tsc::FieldKick, "__str__",
				       pstr);
	}
};
#endif
...
void add_methods_field_kick(py::class_<Class> t_mapper)
{
	...
	t_mapper
		...
		.def("get_dx",                          []
		     (Class &kick){return kick.getTransform()->getDx();})
		.def("get_dy",                          []
		     (Class &kick){return kick.getTransform()->getDy();})
		// .def("get_roll",                        []
...
void add_methods_classical_magnet(py::class_<Class> t_mapper)
{
	...
	t_mapper
		...
		// .def("propagate", py::overload_cast<tsc::ConfigType&,
		//      gtpsa::ss_vect<tps>&>
		//      (&tse::ClassicalMagnet::propagate), pass_tps_doc)
		.def("propagate", py::overload_cast<tsc::ConfigType&,
...
struct TemplatedClasses
{
	...
	(py::class_<tsc::ElemType, PyElemType,
	 std::shared_ptr<tsc::ElemType>>& elem_type){
		...
		py::class_<DriftTypeK,
			   std::shared_ptr<DriftTypeK>>
			drift(this->m_module,
			      drift_type_name.c_str(), elem_type);
		drift
			.def(py::init<const Config &>());

		typedef tse::FieldKickKnobbed<C> FieldKickK;
		std::string field_kick_name = "FieldKick" + this->m_suffix;
		py::class_<FieldKickK, std::shared_ptr<FieldKickK>>
			field_kick(this->m_module, field_kick_name.c_str(),
				   elem_type);
		add_methods_field_kick<C, FieldKickK>(field_kick);
		field_kick
			// .def("getTransform", &tse::FieldKick::getTransform)
			                                   /* causes segfault */
			.def(py::init<const Config &>());
		...
		py::class_<nlk, std::shared_ptr<nlk>>
			(this->m_module, nlk_name.c_str(), field_kick)
			.def("get_field_interpolator",
			     &nlk::getFieldInterpolator)
			.def("set_field_interpolator",
			     &nlk::setFieldInterpolator)
			.def(py::init<const Config &>());
		return elem_type;
	}
};
...
{
	py::class_<tsc::CellVoid, std::shared_ptr<tsc::CellVoid>>
		cell_void(m, "CellVoid");
	cell_void
	...
	py::class_<tsc::ElemTypeKnobbed/*<C>*/, PyElemType/*<C>*/,
		   std::shared_ptr<tsc::ElemTypeKnobbed/*<C>*/>>
		elem_type (m, elem_type_name.c_str(), cell_void);
	elem_type.def(py::init<const Config &>());

	elem_type
		.def("__str__",
		     &tsc::ElemTypeKnobbed/*<C>*/::pstr)
		.def("__repr__",
		     &tsc::ElemTypeKnobbed/*<C>*/::repr)
		.def("get_length",
		     &tsc::ElemTypeKnobbed/*<C>*/::getLength)
		.def("set_length",
		     &tsc::ElemTypeKnobbed/*<C>*/::setLength)
		.def("get_observer",
		     &tsc::ElemTypeKnobbed/*<C>*/::observer)
		.def("set_observer",
		...
		// .def("propagate",
		//      py::overload_cast<tsc::ConfigType&,
		//      gtpsa::ss_vect<tps>&>(&tse::ElemType::propagate),
		//      pass_tpsa_doc)
                ;
	...
	TemplatedClasses<tsc::TpsaVariantType>
		templated_classes_tpsa(m, "Tpsa");
	templated_classes_tpsa.buildClasses(elem_type);

	// classes without knobs follow

	py::class_<tse::MarkerType,
		   std::shared_ptr<tse::MarkerType>>(m, "Marker", elem_type)
		.def("get_radiation_delegate",
	...
	py::class_<tse::BPMType,
		   std::shared_ptr<tse::BPMType>>(m, "BPM", elem_type)
		.def(py::init<const Config &>());


	//, std::shared_ptr<tse::>
	py::class_<tse::CavityType,
		   std::shared_ptr<tse::CavityType>>(m, "Cavity", elem_type)
		.def("set_frequency",
		     &tse::CavityType::setFrequency)
		.def("get_frequency",
		     &tse::CavityType::getFrequency)
		.def("set_voltage",
		     &tse::CavityType::setVoltage)
		.def("get_voltage",
		     &tse::CavityType::getVoltage)
		.def("set_phase",
		     &tse::CavityType::setPhase)
		.def("get_phase",
		     &tse::CavityType::getPhase)
		.def("set_harmonic_number",
		     &tse::CavityType::setHarmonicNumber)
		.def("get_harmonic_number",
		     &tse::CavityType::getHarmonicNumber)
		.def(py::init<const Config &>());
